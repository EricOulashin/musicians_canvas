...
本アプリケーションは、デスクトップPC向けのマルチトラック音楽録音ツールです。C++で記述されており、GUIライブラリとしてQtを採用しています。Linux、Windows、およびMac OSでの動作を想定して開発されています。設計においては、「使いやすさ」を最重要視しています。本プロジェクトは当初、CursorやClaude（AIコーディング支援ツール）を用いた実験としてスタートしました。AIによる開発手法と従来のソフトウェア開発手法を併用することで、こうした種類のアプリケーションをどこまで構築できるか、その可能性を検証することを目的としています。
