...
- **Inspelnings-inserteffekter (ljudspår)**: Knappen **Effects** (under **Options** på varje ljudspår) öppnar dialogen **Track effects**. Lägg till och konfigurera **Reverb**, **Chorus** och **Flanger**, dra **≡**-handtaget för att sortera om kedjan (överst körs först) och spara med projektet. Effekterna tillämpas på tagningen när du stoppar inspelningen; parametrarna är i verkliga enheter (ms, Hz) så att beteendet förblir konsekvent för mono eller stereo och typiska projektsamplingsfrekvenser (8 kHz–192 kHz) efter att inspelningen normaliserats. Se [användarhandboken](docs/MusiciansCanvas_User_Manual_sv.md). Översättare kan massuppdatera strängar via `scripts/effect_i18n.tsv` (genereras av `scripts/build_effect_tsv.py`) och `scripts/fill_effect_i18n.py` efter `lupdate`.
