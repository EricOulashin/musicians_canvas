...
- **Efeitos de mixagem (bus principal)**: **Project → Project Settings → Mix Effects** define uma cadeia no **mix completo** (mesmos tipos e ordem das faixas). Aplicado ao **reproduzir todas as faixas** e ao **exportar o mix** (WAV/FLAC). Salvo em `project.json` como `mixEffectChain`.
- **Limitação suave antes do PCM de 16 bits**: DSP integrado reduz recorte digital duro. **EffectWidget**: `guardFloatSampleForInt16Pcm()` e `softLimitFloatSampleForInt16Pcm()`.
...
- **Adicionar faixa de bateria**: **Tools → Adicionar faixa de bateria** (atalho **D**) adiciona uma faixa MIDI no canal 10 General MIDI (bateria) e grava um `.mid` curto de dois compassos (bumbo, caixa, hi-hat) na pasta do projeto. Se **Metrônomo** tiver **Ativar metrônomo durante a gravação**, usa-se esse BPM; caso contrário, estima-se pelo áudio habilitado ou 120 BPM. Veja o [manual do usuário](docs/MusiciansCanvas_User_Manual_pt-BR.md).
