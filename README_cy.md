...
- **Effeithiau mewnosod wrth recordio (traciau sain yn unig)**: Defnyddiwch y botwm **Effects** (o dan **Options** ar bob trac sain) i agor y ddeialog **Track effects**. Ychwanegwch a ffurfweddu **Reverb**, **Chorus**, **Flanger**, **Overdrive / distortion** a **Amp & cabinet** (modelu amp a chabinet), llusgwch **≡** i aildrefnu'r gadwyn (y brig yn gyntaf), a chadwch gyda'r prosiect. Caiff effeithiau eu cymhwyso at y dyniad pan fyddwch chi'n stopio recordio; mae paramedrau mewn unedau go iawn (ms, Hz) yn cadw ymddygiad cysylltiedig ar gyfer mono neu stereo a chyfraddau samplu prosiect nodweddiadol (8 kHz–192 kHz) ar ôl normalaiddio'r cipio. Gweler y [llawlyfr defnyddiwr](docs/MusiciansCanvas_User_Manual_cy.md). Gall cyfieithwyr ddiweddaru llinynnau mewn swp trwy `scripts/effect_i18n.tsv` (a grëwyd gan `scripts/build_effect_tsv.py`) a `scripts/fill_effect_i18n.py` ar ôl `lupdate`.
- **Effeithiau cymysgu (prif fws)**: **Project → Project Settings → Mix Effects** yn ffurfweddu cadwyn ar y **cymysg cyfan** (yr un fath a'r drefn â thrac). Yn cael ei ddefnyddio wrth **chwarae pob trac** ac **allforio'r gymysg** (WAV/FLAC). Cadwyd yn `project.json` fel `mixEffectChain`.
- **Atal clipio digidol**: DSP mewnol yn cyfyngu'n feddal cyn PCM 16-did. **EffectWidget**: `guardFloatSampleForInt16Pcm()` a `softLimitFloatSampleForInt16Pcm()`.
- **Monitro wrth recordio**: **Monitro sain wrth recordio** (i'r dde o'r display LED) yn anfon **mewnbwn byw** i **allbwn sain y prosiect** yn ystod capio. **Sain** — yr union beth sy'n cael ei recordio; **MIDI** — syntheseisydd meddal gyda rendro MIDI a SoundFont. Caiff ei gadw yn y **prosiect** (`monitorWhileRecording`). Diffoddwch i leihau adborth.
...
- **Ychwanegu trac drymiau**: **Tools → Ychwanegu trac drymiau** (bysellfwrdd byr **D**) yn ychwanegu trac MIDI ar sianel 10 General MIDI (drymiau) ac yn ysgrifennu ffeil `.mid` dwy fesur fer i ffolder y prosiect. Os oes **Metronome** â **Galluogi'r metrôn wrth recordio** ymlaen, defnyddir y BPM hwnnw; fel arall amcangyfrifir o sain wedi'i alluogi neu 120 BPM. Gweler y [llawlyfr defnyddiwr](docs/MusiciansCanvas_User_Manual_cy.md).
