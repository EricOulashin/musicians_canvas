...
Musician's Canvas 是一款適用於桌上型電腦的多軌音樂錄製應用程式。它支援從麥克風和線路輸入裝置進行音訊錄製、從鍵盤和控制器進行 MIDI 錄製，並可將所有音軌混音為單一音訊檔案（WAV、FLAC、MP3、Ogg Vorbis、AIFF）。附屬應用程式 Virtual MIDI Keyboard 提供了一個軟體鋼琴鍵盤，用於傳送 MIDI 音符。
...
- 自動單聲道/立體聲裝置偵測
- 基於專案的設定，可針對個別專案進行覆寫
- 混音匯出為 WAV、FLAC、MP3、Ogg Vorbis、AIFF（可在 **Mix tracks to file** 中為 MP3／Ogg 指定位元率或音質）
- 深色和淺色主題
...
![主視窗](../screenshots/i18n/zh_TW/MusiciansCanvas_1_MainWin.png)

### 按鈕列

選單列正下方有一個帶有快速存取按鈕的工具列：

![按鈕列](../screenshots/i18n/zh_TW/ButtonBar.png)
...
- **支援的格式：** `.wav`、`.flac`、`.mp3`、`.ogg`、`.aiff`、`.aif`、`.aifc`。任何其他格式的檔案會被靜默略過，結束時
  會有一個對話框列出哪些檔案被略過。
...
![音軌設定](../screenshots/i18n/zh_TW/MusiciansCanvas_2_TrackConfig.png)
...
![軌道效果對話框](../screenshots/i18n/zh_TW/Track_Effects_Dialog.png)
...
![Amp & cabinet 建模 (1)](../screenshots/i18n/zh_TW/Amp_And_Cabinet_Model_1.png)

![Amp & cabinet 建模 (2)](../screenshots/i18n/zh_TW/Amp_And_Cabinet_Model_2.png)
...
![節拍器設定](../screenshots/i18n/zh_TW/MetronomeSettings.png)
...
- **輸出檔案**：瀏覽以選擇目的地檔案路徑。
- **格式**：儲存時依副檔名選擇 — **FLAC**（無損壓縮）、**WAV**（未壓縮 PCM）、**MP3**、**Ogg Vorbis**、**AIFF**。有損格式（MP3、Ogg）檔案較小；WAV、FLAC、AIFF 依內容可為無損。
- **編碼**（僅 MP3 與 Ogg Vorbis）：若目的地檔名以 `.mp3`、`.ogg`、`.oga` 結尾，對話框會顯示 **Encoding** 區段。**MP3** 可選 CBR、ABR 或 VBR，並設定位元率或 VBR 品質。**Ogg Vorbis** 可選以品質為主的可變位元率或約略標稱位元率。**WAV**、**FLAC**、**AIFF** 不會顯示此區段。
...
![一般設定](../screenshots/i18n/zh_TW/MusiciansCanvas_3_GeneralSettings.png)
...
![顯示設定](../screenshots/i18n/zh_TW/MusiciansCanvas_DisplaySettings.png)
...
![語言設定](../screenshots/i18n/zh_TW/MusiciansCanvas_LanguageSettings.png)
...
![MIDI 設定](../screenshots/i18n/zh_TW/MusiciansCanvas_4_MIDISettings.png)
...
![音訊設定](../screenshots/i18n/zh_TW/MusiciansCanvas_5_AudioSettings.png)

### 專案設定

使用 **Project > Project Settings**（Ctrl+P）為目前專案覆寫全域預設值。這對於需要特定取樣率、SoundFont 或音訊裝置的專案非常有用。專案特定設定儲存在 `project.json` 檔案中。

![專案 MIDI 設定](../screenshots/i18n/zh_TW/MusiciansCanvas_6_ProjectMIDISettings.png)
...
![專案音訊設定](../screenshots/i18n/zh_TW/MusiciansCanvas_7_ProjectAudioSettings.png)
...
![Virtual MIDI Keyboard](../screenshots/i18n/zh_TW/VMIDIKeyboard1.png)
...
![Virtual MIDI Keyboard 設定](../screenshots/i18n/zh_TW/VMIDIKeyboard2.png)
...
僅在已設定**專案目錄**，且 Musician's Canvas **未在播放或錄製**時才接受拖放。支援 **`.wav`**、**`.flac`**、**`.mp3`**、**`.ogg`**、**`.aiff`**、**`.aif`**、**`.aifc`**；其他副檔名會略過並在對話框列出。每個檔案成為新**音訊**音軌，名稱取自檔名（不含副檔名）。
