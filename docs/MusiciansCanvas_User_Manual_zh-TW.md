...
可加入 **殘響**、**合唱**、**鑲邊（Flanger）**、**過載／失真（Overdrive / distortion）** 與 **音箱頭與箱體建模（Amp & cabinet）**（箱頭類型、箱體、增益、低／中／高頻、空氣感與溼乾比等）。以下為 **Amp & cabinet** 的介面範例：
...
- **Beats per minute**：速度的數字輸入，以每分鐘拍數（BPM）為單位。範圍為 20–300 BPM。
