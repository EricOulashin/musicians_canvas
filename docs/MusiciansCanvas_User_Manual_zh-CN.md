...
Musician's Canvas 是一款适用于桌面电脑的多轨音乐录音应用程序。它支持通过麦克风和线路输入设备进行音频录制，通过键盘和控制器进行 MIDI 录制，并可将所有轨道混合导出为单个音频文件（WAV、FLAC、MP3、Ogg Vorbis、AIFF）。配套应用程序 Virtual MIDI Keyboard 提供了一个软件钢琴键盘，用于发送 MIDI 音符。
...
- 自动检测单声道/立体声设备
- 基于项目的设置，支持按项目覆盖
- 混合导出为 WAV、FLAC、MP3、Ogg Vorbis、AIFF（在 **Mix tracks to file** 中可为 MP3／Ogg 指定比特率或音质）
- 深色和浅色主题
...
![主窗口](../screenshots/i18n/zh_CN/MusiciansCanvas_1_MainWin.png)

### 按钮栏

菜单栏正下方有一个带有快速访问按钮的工具栏：

![按钮栏](../screenshots/i18n/zh_CN/ButtonBar.png)
...
- **支持的格式：** `.wav`、`.flac`、`.mp3`、`.ogg`、`.aiff`、`.aif`、`.aifc`。任何其他格式的文件会被静默跳过，
  结束时会有一个对话框列出哪些文件被跳过。
...
![轨道配置](../screenshots/i18n/zh_CN/MusiciansCanvas_2_TrackConfig.png)
...
![轨道效果对话框](../screenshots/i18n/zh_CN/Track_Effects_Dialog.png)
...
![Amp & cabinet 建模 (1)](../screenshots/i18n/zh_CN/Amp_And_Cabinet_Model_1.png)

![Amp & cabinet 建模 (2)](../screenshots/i18n/zh_CN/Amp_And_Cabinet_Model_2.png)
...
![节拍器设置](../screenshots/i18n/zh_CN/MetronomeSettings.png)
...
- **输出文件**：浏览并选择目标文件路径。
- **格式**：保存时通过扩展名选择 — **FLAC**（无损压缩）、**WAV**（未压缩 PCM）、**MP3**、**Ogg Vorbis**、**AIFF**。有损格式（MP3、Ogg）文件更小；WAV、FLAC、AIFF 依内容可为无损。
- **编码**（仅 MP3 与 Ogg Vorbis）：若目标文件名以 `.mp3`、`.ogg`、`.oga` 结尾，对话框会显示 **Encoding** 区域。**MP3** 可选择 CBR、ABR 或 VBR，并设置比特率或 VBR 质量。**Ogg Vorbis** 可选择基于质量的可变比特率或近似标称比特率。**WAV**、**FLAC**、**AIFF** 不显示该区域。
...
![常规设置](../screenshots/i18n/zh_CN/MusiciansCanvas_3_GeneralSettings.png)
...
![显示设置](../screenshots/i18n/zh_CN/MusiciansCanvas_DisplaySettings.png)
...
![语言设置](../screenshots/i18n/zh_CN/MusiciansCanvas_LanguageSettings.png)
...
![MIDI 设置](../screenshots/i18n/zh_CN/MusiciansCanvas_4_MIDISettings.png)
...
![音频设置](../screenshots/i18n/zh_CN/MusiciansCanvas_5_AudioSettings.png)

### 项目设置

使用 **Project > Project Settings**（Ctrl+P）仅为当前项目覆盖全局默认值。这对于需要特定采样率、SoundFont 或音频设备的项目非常有用。项目特定设置保存在 `project.json` 文件中。

![项目 MIDI 设置](../screenshots/i18n/zh_CN/MusiciansCanvas_6_ProjectMIDISettings.png)
...
![项目音频设置](../screenshots/i18n/zh_CN/MusiciansCanvas_7_ProjectAudioSettings.png)
...
![Virtual MIDI Keyboard](../screenshots/i18n/zh_CN/VMIDIKeyboard1.png)
...
![Virtual MIDI Keyboard 配置](../screenshots/i18n/zh_CN/VMIDIKeyboard2.png)
...
仅在已设置**项目目录**，且 Musician's Canvas **未在播放或录制**时才能接受拖放。支持的格式包括 **`.wav`**、**`.flac`**、**`.mp3`**、**`.ogg`**、**`.aiff`**、**`.aif`**、**`.aifc`**；其他扩展名会被跳过并在对话框中列出。每个文件会成为一个新的**音频**轨道，名称取自文件名（不含扩展名）。
