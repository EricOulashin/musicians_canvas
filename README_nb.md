...
- **Innsettings-effekter ved opptak (lydspor)**: Knappen **Effects** (under **Options** på hvert lydspor) åpner dialogen **Track effects**. Legg til og konfigurer **Reverb**, **Chorus** og **Flanger**, dra **≡**-grepet for å endre rekkefølgen på kjeden (øverst kjører først), og lagre med prosjektet. Effektene brukes på opptaket når du stopper opptaket; parametre i virkelige enheter (ms, Hz) gir konsistent oppførsel for mono eller stereo og typiske prosjektsamplingsfrekvenser (8 kHz–192 kHz) etter normalisering av opptaket. Se [brukerhåndboken](docs/MusiciansCanvas_User_Manual_nb.md). Oversettere kan også oppdatere strenger i bulk via `scripts/effect_i18n.tsv` (generert av `scripts/build_effect_tsv.py`) og `scripts/fill_effect_i18n.py` etter `lupdate`.
