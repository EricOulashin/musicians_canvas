...
- **Miks-effekter (hovedbuss)**: **Project → Project Settings → Mix Effects** setter en kjed på **hele miksen** (samme typer og rekkefølge som på spor). Brukes ved **avspilling av alle spor** og **eksport av miks** (WAV/FLAC). Lagres i `project.json` som `mixEffectChain`.
- **Myk begrensning før 16-bit PCM**: Innebygd DSP reduserer hard digital klipping. **EffectWidget** tilbyr `guardFloatSampleForInt16Pcm()` og `softLimitFloatSampleForInt16Pcm()`.
...
- **Legg til trommespor**: **Tools → Legg til trommespor** (snarvei **D**) legger til et MIDI-spor på General MIDI kanal 10 (trommer) og skriver en kort to-takts `.mid` (kick, snare, hi-hat) i prosjektmappa. Hvis **Metronom** har **Aktiver metronom under opptak** på, brukes den BPM; ellers estimeres tempo fra aktivert lyd eller 120 BPM. Se [brukerhåndboken](docs/MusiciansCanvas_User_Manual_nb.md).
