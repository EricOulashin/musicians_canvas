<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE TS>
<TS version="2.1" language="el_GR">
<context>
    <name>VkConfigDialog</name>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="43"/>
        <source>Configuration</source>
        <translation>Διαμόρφωση</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="54"/>
        <source>Apply</source>
        <translation>Εφαρμογή</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="69"/>
        <source>Here, you can configure settings related to MIDI.</source>
        <translation>Εδώ μπορείτε να ρυθμίσετε τις ρυθμίσεις που σχετίζονται με το MIDI.</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="71"/>
        <source>MIDI Output Device</source>
        <translation>Συσκευή Εξόδου MIDI</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="80"/>
        <source>MIDI Input Device</source>
        <translation>Συσκευή Εισόδου MIDI</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="87"/>
        <source>SoundFont (for software synthesizer)</source>
        <translation>SoundFont (για συνθετητή λογισμικού)</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="90"/>
        <source>Path to .sf2 SoundFont file</source>
        <translation>Διαδρομή αρχείου SoundFont .sf2</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="92"/>
        <source>Browse...</source>
        <translation>Αναζήτηση...</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="97"/>
        <source>Refresh</source>
        <translation>Ανανέωση</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="101"/>
        <source>Synthesizer Volume (Master Gain)</source>
        <translation>Ένταση Συνθετητή (Κύρια Ενίσχυση)</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="106"/>
        <source>Quiet</source>
        <translation>Χαμηλά</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="116"/>
        <source>Loud</source>
        <translation>Δυνατά</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="129"/>
        <source>MIDI</source>
        <translation>MIDI</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="140"/>
        <source>Here, you can configure audio settings for the application; specifically, for any virtual MIDI synthesizer selected.</source>
        <translation>Εδώ μπορείτε να ρυθμίσετε τις ρυθμίσεις ήχου για την εφαρμογή· συγκεκριμένα, για οποιονδήποτε εικονικό συνθετητή MIDI έχει επιλεγεί.</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="143"/>
        <source>Audio Output Device</source>
        <translation>Συσκευή Εξόδου Ήχου</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="151"/>
        <source>Audio</source>
        <translation>Ήχος</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="166"/>
        <source>Choose the display language for the application.</source>
        <translation>Επιλέξτε τη γλώσσα εμφάνισης της εφαρμογής.</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="168"/>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="195"/>
        <source>Language</source>
        <translation>Γλώσσα</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="172"/>
        <source>System Default</source>
        <translation>Προεπιλογή Συστήματος</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="209"/>
        <source>Built-in Software Synthesizer</source>
        <translation>Ενσωματωμένος Συνθετητής Λογισμικού</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="225"/>
        <source>(None)</source>
        <translation>(Κανένα)</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="241"/>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="246"/>
        <source>(Default)</source>
        <translation>(Προεπιλογή)</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="314"/>
        <source>Select SoundFont</source>
        <translation>Επιλογή SoundFont</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_configdialog.cpp" line="315"/>
        <source>SoundFont (*.sf2 *.SF2);;All files (*)</source>
        <translation>SoundFont (*.sf2 *.SF2);;Όλα τα αρχεία (*)</translation>
    </message>
</context>
<context>
    <name>VkHelpDialog</name>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_helpdialog.cpp" line="7"/>
        <source>Using this application</source>
        <translation>Χρήση αυτής της εφαρμογής</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_helpdialog.cpp" line="13"/>
        <source>&lt;h3&gt;Playing the piano keyboard&lt;/h3&gt;&lt;p&gt;You can play notes using either the mouse or the computer keyboard.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Mouse:&lt;/b&gt; Click and hold on a piano key to play a note. The note will continue playing while the left mouse button is held down. Release the mouse button to stop the note.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Keyboard:&lt;/b&gt; Press and release the computer keyboard keys mapped to piano keys. The keys are mapped similarly to a piano - for example, the A through L keys play the white keys (C major scale), and the W, E, T, Y, U keys play the black keys (sharps/flats). The octave can be adjusted using the Octave control in the toolbar.&lt;/p&gt;</source>
        <translation>&lt;h3&gt;Παίζοντας το πληκτρολόγιο πιάνου&lt;/h3&gt;&lt;p&gt;Μπορείτε να παίξετε νότες χρησιμοποιώντας είτε το ποντίκι είτε το πληκτρολόγιο του υπολογιστή.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Ποντίκι:&lt;/b&gt; Κάντε κλικ και κρατήστε πατημένο σε ένα πλήκτρο πιάνου για να παίξετε μια νότα. Η νότα θα συνεχίσει να παίζει ενώ το αριστερό κουμπί του ποντικιού είναι πατημένο. Αφήστε το κουμπί του ποντικιού για να σταματήσετε τη νότα.&lt;/p&gt;&lt;p&gt;&lt;b&gt;Πληκτρολόγιο:&lt;/b&gt; Πατήστε και αφήστε τα πλήκτρα του πληκτρολογίου του υπολογιστή που αντιστοιχούν στα πλήκτρα πιάνου. Τα πλήκτρα αντιστοιχίζονται παρόμοια με ένα πιάνο - για παράδειγμα, τα πλήκτρα A έως L παίζουν τα λευκά πλήκτρα (κλίμακα Ντο μείζονα), και τα πλήκτρα W, E, T, Y, U παίζουν τα μαύρα πλήκτρα (διέσεις/υφέσεις). Η οκτάβα μπορεί να ρυθμιστεί χρησιμοποιώντας το χειριστήριο Οκτάβας στη γραμμή εργαλείων.&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_helpdialog.cpp" line="27"/>
        <source>&lt;h3&gt;Configuration&lt;/h3&gt;&lt;p&gt;Use Settings → Configuration to configure the application:&lt;/p&gt;&lt;ul&gt;&lt;li&gt;&lt;b&gt;MIDI tab:&lt;/b&gt; Select the MIDI output device (physical port or built-in software synthesizer), MIDI input device for forwarding external MIDI, and SoundFont file for the software synthesizer.&lt;/li&gt;&lt;li&gt;&lt;b&gt;Audio tab:&lt;/b&gt; Select the audio output device used when the built-in software synthesizer is selected.&lt;/li&gt;&lt;/ul&gt;</source>
        <translation>&lt;h3&gt;Διαμόρφωση&lt;/h3&gt;&lt;p&gt;Χρησιμοποιήστε Ρυθμίσεις → Διαμόρφωση για να ρυθμίσετε την εφαρμογή:&lt;/p&gt;&lt;ul&gt;&lt;li&gt;&lt;b&gt;Καρτέλα MIDI:&lt;/b&gt; Επιλέξτε τη συσκευή εξόδου MIDI (φυσική θύρα ή ενσωματωμένος συνθετητής λογισμικού), τη συσκευή εισόδου MIDI για προώθηση εξωτερικού MIDI και το αρχείο SoundFont για τον συνθετητή λογισμικού.&lt;/li&gt;&lt;li&gt;&lt;b&gt;Καρτέλα Ήχου:&lt;/b&gt; Επιλέξτε τη συσκευή εξόδου ήχου που χρησιμοποιείται όταν είναι επιλεγμένος ο ενσωματωμένος συνθετητής λογισμικού.&lt;/li&gt;&lt;/ul&gt;</translation>
    </message>
</context>
<context>
    <name>VkMainWindow</name>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="35"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="48"/>
        <source>Virtual MIDI Keyboard</source>
        <translation>Εικονικό Πληκτρολόγιο MIDI</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="38"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="109"/>
        <source>Volume</source>
        <translation>Ένταση</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="39"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="137"/>
        <source>Octave</source>
        <translation>Οκτάβα</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="40"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="176"/>
        <source>Chorus</source>
        <translation>Χορωδία</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="41"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="178"/>
        <source>Apply</source>
        <translation>Εφαρμογή</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="42"/>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="188"/>
        <source>Virtual MIDI sound:</source>
        <translation>Εικονικός ήχος MIDI:</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="73"/>
        <source>&amp;File</source>
        <translation>&amp;Αρχείο</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="74"/>
        <source>&amp;Close</source>
        <translation>&amp;Κλείσιμο</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="78"/>
        <source>&amp;Settings</source>
        <translation>&amp;Ρυθμίσεις</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="79"/>
        <source>&amp;Configuration</source>
        <translation>&amp;Διαμόρφωση</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="83"/>
        <source>&amp;Help</source>
        <translation>&amp;Βοήθεια</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="84"/>
        <source>&amp;About</source>
        <translation>&amp;Σχετικά</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="86"/>
        <source>&amp;Using this application</source>
        <translation>&amp;Χρήση αυτής της εφαρμογής</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="113"/>
        <source>&lt;</source>
        <translation>&lt;</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="123"/>
        <source>&gt;</source>
        <translation>&gt;</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="248"/>
        <source>&lt;h3&gt;Virtual MIDI Keyboard %1&lt;/h3&gt;&lt;p&gt;An on-screen virtual MIDI keyboard with a built-in FluidSynth software synthesizer. Play notes with the mouse or computer keyboard, send MIDI to external devices, or forward incoming MIDI from a connected controller.&lt;/p&gt;&lt;p&gt;Companion application to Musician&apos;s Canvas. Written in C++ with Qt6.&lt;/p&gt;&lt;p&gt;Copyright &amp;copy; Eric Oulashin&lt;/p&gt;&lt;p&gt;&lt;a href=&quot;%2&quot;&gt;%2&lt;/a&gt;&lt;/p&gt;</source>
        <translation>&lt;h3&gt;Virtual MIDI Keyboard %1&lt;/h3&gt;&lt;p&gt;Ένα εικονικό πληκτρολόγιο MIDI στην οθόνη με ενσωματωμένο συνθεσάιζερ λογισμικού FluidSynth. Παίξτε νότες με το ποντίκι ή το πληκτρολόγιο του υπολογιστή, στείλτε MIDI σε εξωτερικές συσκευές ή προωθήστε εισερχόμενο MIDI από συνδεδεμένο χειριστήριο.&lt;/p&gt;&lt;p&gt;Συμπληρωματική εφαρμογή στο Musician&apos;s Canvas. Γραμμένο σε C++ με Qt6.&lt;/p&gt;&lt;p&gt;Copyright &amp;copy; Eric Oulashin&lt;/p&gt;&lt;p&gt;&lt;a href=&quot;%2&quot;&gt;%2&lt;/a&gt;&lt;/p&gt;</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="260"/>
        <source>About Virtual MIDI Keyboard</source>
        <translation>Σχετικά με το Virtual MIDI Keyboard</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="369"/>
        <source>(No SoundFont — configure one in Settings)</source>
        <translation>(Δεν υπάρχει SoundFont — ρυθμίστε ένα στις Ρυθμίσεις)</translation>
    </message>
    <message>
        <location filename="../src/virtual_midi_keyboard/vk_mainwindow.cpp" line="399"/>
        <source>Audio driver failed to initialise. Check that PulseAudio or PipeWire is running.</source>
        <translation>Ο οδηγός ήχου απέτυχε να αρχικοποιηθεί. Ελέγξτε ότι λειτουργεί το PulseAudio ή το PipeWire.</translation>
    </message>
</context>
</TS>
