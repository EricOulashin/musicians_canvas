...
這是一個適用於桌上型電腦的多軌音樂錄製應用程式。它使用 C++ 編寫，並使用 Qt 作為圖形使用者介面庫。該應用程式旨在支援 Linux、Windows 和 macOS 系統。易用性是其主要設計考量。該專案最初是使用 Cursor & Claude（人工智慧編碼工具）進行的一項實驗，旨在探索在人工智慧開發和傳統軟體開發相結合的情況下，它能夠創建出怎樣的應用程式。
